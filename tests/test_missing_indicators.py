...
from validation.data import add_missing_indicators, prepare_dataset
...
def frame():
    ...
    df.loc[:2, "new_dense"] = np.nan                        # ~0.1%, below threshold
    return df
...
def test_treat_as_source_follows_the_originating_column(frame):
    dataset = prepare_dataset(frame, _config(scope="all", treat_as="source"))
...
def test_columns_that_carry_no_information_are_skipped(frame):
    ...
    dataset = prepare_dataset(frame, _config(scope="all", min_missing_rate=0.0))
    ...
    dataset = prepare_dataset(always, _config(scope="candidates", min_missing_rate=0.0))
    assert "new_gone_is_missing" not in _indicators(dataset)


def test_min_missing_rate_is_respected(frame):
    assert "new_dense_is_missing" not in _indicators(prepare_dataset(frame, _config()))
    loose = prepare_dataset(frame, _config(min_missing_rate=0.0005))
...
def test_suffix_is_configurable(frame):
    dataset = prepare_dataset(frame, _config(suffix="__nan"))
