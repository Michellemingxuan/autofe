...
from mllite.stages.feature_selection import build_verdict_table, run_feature_selection
...
def frame():
    return make_frame(n_rows=9_000, seed=6)
...
def test_every_candidate_gets_exactly_one_verdict(frame, tmp_path):
...
def test_verdict_names_the_gate_that_decided(frame, tmp_path):
    cfg = _config(tmp_path)
    dataset = prepare_dataset(frame, cfg)
    result = run_feature_selection(dataset, cfg)
...
def test_kept_candidates_are_listed_first(frame, tmp_path):
    cfg = _config(tmp_path)
    dataset = prepare_dataset(frame, cfg)
...
def test_a_candidate_cut_by_data_quality_still_gets_a_verdict(frame, tmp_path):
    """The case that used to disappear: dropped before the screens ever ran."""
    frame = frame.assign(new_constant=1.0)
    cfg = _config(tmp_path)
...
def test_verdicts_are_written_and_summarised(frame, tmp_path):
...
def test_selection_disabled_lets_every_candidate_through(frame, tmp_path):
    cfg = _config(tmp_path)
